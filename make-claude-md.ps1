...
# impossible (test command and build tooling are project-specific), so the loop
...
# Guard: CLAUDE.md is generated by make-claude-md.ps1 and is what gives `claude -p`
# its coordination context. Without it the agent has no protocol and will freelance.
if (-not (Test-Path "CLAUDE.md")) {
    Write-Error "CLAUDE.md not found. Run .\make-claude-md.ps1 first — it generates CLAUDE.md, .claude\, and this script. Then start .\run.ps1 again."
    exit 1
}
...
while ($true) {
    ...
    $output = claude -p "Go" $SkipPerms 2>&1 | Out-String
    Write-Host $output
    Write-Host ""
    ...
    if ($output -match 'NOTHING_TO_DO') {
        ...
        $IDLE++
        if ($IDLE -ge $MAX_IDLE) {
            Write-Host "NOTHING_TO_DO x$MAX_IDLE consecutive. Project complete — stopping."
            break
        }
        Write-Host "Idle ($IDLE/$MAX_IDLE). Waiting ${IDLE_WAIT}s..."
        Start-Sleep -Seconds $IDLE_WAIT
    } elseif ($output -match 'WAITING_FOR_PEERS') {
        ...
        $IDLE = 0
        Write-Host "Peers still working. Waiting ${PEER_WAIT}s (not counted as idle)..."
        Start-Sleep -Seconds $PEER_WAIT
    } else {
        # Did a real unit of work — go again promptly with a fresh context.
        $IDLE = 0
        Start-Sleep -Seconds $WORK_WAIT
    }
}
