...
3. **Act**:
   - `mcp__github__update_issue`: assign self + label **[target]** (caller specifies: `in-progress` for task claims, `in-review` for review claims).
   - `mcp__github__add_issue_comment`: `agent: claiming - [user] - [timestamp]`.
4. **Verify**: Re-read issue. Conflict? -> Unassign + Reset label to prior state + ABORT.
